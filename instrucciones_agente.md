...
- ✅ **Multiplataforma** (Windows x64, Linux x64)
- ✅ Integrable con **Serena** vía wrapper Python
...
### 7.1 Wrapper Python para Serena

Crea `src/solidlsp/language_servers/mql4_language_server.py` (en Serena):

```python
"""
MQL4 Language Server wrapper for Serena.
"""
import logging
import os
import platform
...
class MQL4LanguageServer(SolidLanguageServer):
    ...
    Uses the MQL4 Language Server implemented in C# .NET 8.
    """
    ...
    def _ensure_server_installed(
        ...
        """Ensure MQL4 LSP is installed."""

        # 1. Check if system installation exists
        system_server = shutil.which("mql4-lsp-server")
        if system_server:
            return [system_server]

        # 2. Check if installed via dotnet tool
        try:
            ...
            if "mql4-language-server" in result.stdout:
                return ["dotnet", "tool", "run", "mql4-lsp-server", "--stdio"]
        except FileNotFoundError:
            pass

        # 3. Check local installation
        ls_resources_dir = cls.ls_resources_dir(solidlsp_settings, mkdir=True)
        ...
        if sys == "windows":
            exe_name = "mql4-lsp-server.exe"
        else:
            exe_name = "mql4-lsp-server"
        ...
        server_path = os.path.join(ls_resources_dir, exe_name)

        # If not exists, inform the user
        if not os.path.exists(server_path):
            logger.log(
                f"MQL4 LSP not found. Please install it:\n"
                f"  - .NET: dotnet tool install --global mql4-language-server\n"
                f"  - Or download from: https://github.com/YOUR_USERNAME/mql4-language-server/releases\n"
                f"  - And place the binary in: {server_path}",
                logging.ERROR,
            )
            raise RuntimeError("MQL4 LSP not installed. Install with: dotnet tool install --global mql4-language-server")
    ...
    def __init__(
        ...
        cmd = self._ensure_server_installed(logger, solidlsp_settings)
    ...
    def _start_server(self):
        """Start the MQL4 language server."""

        def on_log_message(msg):
            self.logger.log(f"LSP: {msg}", logging.INFO)
        ...
        # Register handlers
        self.server.on_request("initialize", on_initialize)
        ...
        response = self.server.send.initialize(init_params)
        self.server.notify.initialized({})
        self.completions_available.set()
        self.server_ready.set()
```

### 7.2 Registrar en ls_config.py

Edita `src/solidlsp/ls_config.py`:
...
Crea `test/solidlsp/mql4/test_mql4_basic.py`:

```python
import pytest
from pathlib import Path

from serena.config import LanguageServerConfig
...
@pytest.mark.mql4
@pytest.mark.skip(reason="Requires mql4-lsp-server to be installed")
class TestMQL4Basic:
    """Basic tests for MQL4 language support."""

    def setup_method(self):
        """Setup test environment."""
        self.test_repo_path = Path(__file__).parent.parent.parent / "resources" / "repos" / "mql4" / "test_repo"
        self.config = LanguageServerConfig(code_language=Language.MQL4)
        self.logger = LanguageServerLogger()

    def test_find_symbols(self):
        """Test finding symbols in MQL4 files."""
        with SolidLanguageServer.create(self.config, self.logger, str(self.test_repo_path)) as ls:
            symbols, _ = ls.request_document_symbols("Experts/MyEA.mq4")
            assert len(symbols) > 0

            # Verificar que encuentra OnInit
            on_init = [s for s in symbols if s["name"] == "OnInit"]
            assert len(on_init) == 1
            assert on_init[0]["kind"] == 12  # Function

    def test_find_references(self):
        """Test finding references within file."""
        with SolidLanguageServer.create(self.config, self.logger, str(self.test_repo_path)) as ls:
            # Buscar referencias a una variable
            references = ls.request_references("Experts/MyEA.mq4", line=10, column=0)
    ...
    def test_cross_file_references(self):
        ...
        with SolidLanguageServer.create(self.config, self.logger, str(self.test_repo_path)) as ls:
            # Verificar referencias desde MyEA.mq4 a Include/Common.mqh
            references = ls.request_references("Experts/MyEA.mq4", line=15, column=0)
            assert references is not None

            # Verificar que al menos una referencia está en archivo .mqh
            mqh_refs = [r for r in references if r["relativePath"].endswith(".mqh")]
            assert len(mqh_refs) > 0

    def test_predefined_functions(self):
        """Test detecting MQL4 predefined functions."""
        with SolidLanguageServer.create(self.config, self.logger, str(self.test_repo_path)) as ls:
            symbols, _ = ls.request_document_symbols("Experts/MyEA.mq4")

            # Verificar funciones predefinidas
            predefined_funcs = ["OnInit", "OnTick", "OnDeinit"]
            for func_name in predefined_funcs:
                func = [s for s in symbols if s["name"] == func_name]
                assert len(func) == 1, f"Predefined function {func_name} not found"

    def test_completion(self):
        """Test completion provider."""
        with SolidLanguageServer.create(self.config, self.logger, str(self.test_repo_path)) as ls:
            completions = ls.request_completions("Experts/MyEA.mq4", line=5, column=2)
            assert completions is not None
            assert len(completions) > 0

            # Verificar que incluye keywords MQL4
            completion_texts = [c["completionText"] for c in completions]
            assert "int" in completion_texts
            assert "double" in completion_texts
```

### 7.4 Añadir Marker en pytest
...
markers = [
    # ... existing markers ...
    mql4 = "marks tests as MQL4 language tests",
]
```
